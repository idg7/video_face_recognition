...
from vgg16decoder import Vgg16Decoder, DecoderOnly, Identity
...
def video_emb_rdm(model: nn.Module, dataset_dl: torch.utils.data.DataLoader) -> pd.DataFrame:
    running_loss = 0.0
    ...
    for x, key in pbar:
        ...
        video_embeddings.append(feats)

    print(keys)

    video_embeddings = torch.cat(video_embeddings)
    video_embeddings = video_embeddings / video_embeddings.norm(dim=1, p=2)[:, None]
    rdm = 1 - torch.mm(video_embeddings, video_embeddings.transpose(0, 1))
...
def next_frame_training():
    ...
    # test_ds = TestDataset(f'/home/ssd_storage/datasets/motion_seminar/mtcnn/stills_as_vids', test_frame_transformer, test_video_transformer, num_frames=1, test=True)
    

    rdm_dl = DataLoader(test_ds, batch_size=1, shuffle=True, collate_fn=collate_fn_rnn, num_workers=4)

    with torch.no_grad():
        rdm = video_emb_rdm(model, rdm_dl)
        rdm.to_csv('/home/administrator/PycharmProjects/PyTorch-Computer-Vision-Cookbook/Chapter10/results/video_emb_rdm.csv')
        mlflow.log_artifact('/home/administrator/PycharmProjects/PyTorch-Computer-Vision-Cookbook/Chapter10/results/video_emb_rdm.csv')
